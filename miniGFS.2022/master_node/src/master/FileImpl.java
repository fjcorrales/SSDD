...
public class FileImpl extends UnicastRemoteObject implements File {
	...
	private ManagerImpl manager;					//El manager
	private int repFact;						//Número de réplicas
	private int tam = 0;						//El tamaño del fichero
	...
	*/
	...
	public List <Chunk> getChunkDescriptors(int nchunk, int size) throws RemoteException {
		...
		for(int i = nchunk ; i<nchunk+size ; i++){
			/*
			* comprobamos tal y como dice el enunciado, si existe en el mapa, añadimos a la lista que devolvemos
			* en caso contrario, comprobamos también que estmos dentro del fichero y en este caso ponemos un null
			* es decir, hay un hueco en el fichero, si no cumple ninguno de estos casos, no hacemos nada
			*/
	...
	public List <Chunk> allocateChunkDescriptors(int nchunk, int size) throws RemoteException {
		...
		/*
		* Definimos el tamaño máximo del fichero en cuestion
		*/
		if(tam<(nchunk+size)){
			tam = nchunk+size;
		}
