...
void *servicio(void *arg){
	...
	char tam[100], buff[sizeof(int)], tema[250];
        sockfd=(long) arg;
	...
	while((fgets(tema, 250, f))!=NULL){//while para ir sacando linea por linea los temas
		char *topic = strdup(tema);
		if(set_add(temasList, topic)!=0){
			printf("TEMA NO AÑADIDO\n");
		}else{
			printf("TEMA AÑADIDO-> %s", tema);
		}
	}
	close(f);
	...
	while(1){//espera activa para peticiones
		...
		recv(sockfd, buff, sizeof(int), 0);//guardar peticion en el buffer
